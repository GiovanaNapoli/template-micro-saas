...
import { handleStripeCancelSubscription } from "@/app/server/stripe/handle-delete";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!signature || !secret) {
      return new Response("Webhook signature missing or invalid", {
        status: 400,
      });
    }
    ...
    switch (event.type) {
      case "checkout.session.completed": // pagamento realizado
        ...
        if (metadata?.price === process.env.STRIPE_PRODUCTT_PRICE_ID) {
